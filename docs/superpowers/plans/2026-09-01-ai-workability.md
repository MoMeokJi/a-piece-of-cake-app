...
`flutter run`으로 실행해 확인한다.

1. 자유일기 작성 + 이미지 첨부(2장 이상) → 저장 성공
...
```bash
git add -A
git commit -m "refactor: 멀티파트 API를 dio로 전환하고 http 제거

FormData로 이관. 401 재시도 시 본문은 인터셉터가 clone하여 재전송한다.
BaseApi 삭제, ApiException의 http 팩토리 제거."
```
...
1. 설정 → 탈퇴
2. 회원가입 화면으로 이동하는지
3. 재가입 후 일기 목록이 비어 있는지 (이전 사용자 데이터가 남아 있지 않아야 한다)
