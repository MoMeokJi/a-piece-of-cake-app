...
2. **전체 데이터 삭제 (설정 → '전체 데이터 삭제', `UserApiImpl.deleteUser`).** 코드에서는 `withdraw`/`deleteUser`로 부르지만 앱 UI 문구는 '전체 데이터 삭제'다. 실제 동작은 서버 계정 삭제(`DELETE /users`) + FCM 토큰 삭제 + 로컬 일기 삭제다.
   로그인 상태에서 설정 → '전체 데이터 삭제'를 실행한다. 이 메서드는 직접 작성한 `validateStatus`(`status == 204 || status == 404`)를 쓰는 유일한 곳이고, 성공 시 JWT를 지우는 유일한 경로다.
   **문제가 있다면:** 삭제가 끝나지 않거나(스피너가 멈추지 않음/성공해야 할 상황에서 에러 토스트), 삭제는 성공했는데 앱이 로그인/가입 화면으로 돌아가지 않는다 — 토큰이 실제로는 지워지지 않아 세션이 남아있다는 뜻이다. 가능하다면 서버에서 이미 삭제된 계정 상태로 한 번 더 삭제를 시도해 404-성공 처리 분기도 확인한다 — 이때 404가 에러로 처리되면 문제다.
...
- [ ] **Step 7: 실기기 확인 — 전체 데이터 삭제**

**구현 에이전트는 실기기/시뮬레이터에 접근할 수 없어 이 단계를 수행하지 못했다. 사람이 아래 체크리스트로 직접 확인해야 한다.** 어느 항목도 검증 완료로 표시하지 않았다.

`flutter run`으로 실행해 확인한다. 가장 위험한 증상은 데이터 삭제 후 재가입했을 때 일기 탭 ViewModel(`DiaryCalendarViewModel`, `DiaryListViewModel`)이 이전 계정 데이터를 그대로 들고 있는 것이다 — `resetDiaryTabViewModels()`가 `getIt`에서 재등록만 하고, 이미 화면에 붙어 있는 옛 인스턴스가 여전히 참조되고 있다면 증상이 재현된다.

1. 계정 A로 로그인한 상태에서 일기를 1개 이상 작성한다 (캘린더 탭과 목록 탭 양쪽에서 보이는지 확인)
2. 설정 → '전체 데이터 삭제'를 실행한다
3. 삭제 직후 회원가입 화면으로 이동하는지 확인한다 (에러 토스트 없이 정상 전환)
...
9. 삭제 도중 앱을 백그라운드로 보냈다가 복귀시켜도 크래시나 예외 토스트가 없는지 확인한다
...
3. **문답일기 생성 마지막 단계(`QnaDiaryEditViewModel`)에서 이미지 추가 → 확정 저장 성공.** 이 화면은 서버가 생성해준 글을 다듬고 이미지를 붙여 확정하는 단계다. 폴더 이름이 `qna_diary_edit`이지만 기존 일기를 수정하는 화면이 아니다 — 기존 일기 수정은 상세 화면의 텍스트 수정(`editDiaryText`)뿐이고 이미지 편집 기능은 없다.
